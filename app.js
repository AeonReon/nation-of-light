...
(function () {
  ...
  /* the music (v81): every piece on the shelf, one after another with a stretch of the room alone
     ...
     v89, his: EVERY visit opens on the nocturne ("it anchors the feeling, and I miss it when I first go
     in"), and only staying on brings the others. `opening` = the first music of a visit; after it the
     room walks through `music.pieces` (never the nocturne twice in a row), and the place in that walk is
     kept in S.musicN, so tomorrow's second piece is not today's. */
  ...
  /* index into musicList() of the next piece after the nocturne: 1..n, carried on from the last visit */
  const nextPiece = () => { const n = ((C.music && C.music.pieces) || []).length; if (!n) return 0;
    S.musicN = ((S.musicN || 0) + 1) % n; save(); return 1 + S.musicN; };
  ...
  function musicStart(v, opening) { if (!S.sound || S.musicOff) return; MUSWANT = true; MUSV = v || .55; MUSI = opening ? 0 : nextPiece(); musicPlay(); }
  MUS.addEventListener('ended', () => { if (!MUSWANT) return; MUSI = nextPiece(); const G = (C.music && C.music.gap) || [20, 40]; MUSGAP = setTimeout(musicPlay, (G[0] + Math.random() * (G[1] - G[0])) * 1000); });
  ...
  /* v89: home MODE is not home SHOWING. A ladder, room or area opened from home keeps the stage in
     home mode, so anything that redraws after an action asks this, not atHome(). */
  const homeShowing = () => atHome() && !TRK && !CAT && !AREA && SEARCH === null;
  ...
  function cover() {
    ...
    v.querySelector('#begin').addEventListener('click', () => {
      ac(); unlockAudio();
      musicStart(null, true); ambStart();   /* v89: every visit opens on the dawn nocturne */
  ...
  function codePanel(fromCover) {
    ...
    v.querySelector('#codeform').addEventListener('submit', e => { e.preventDefault(); sfx('tap'); S.code = new FormData(e.target).get('code').trim(); save();
      ...
      if (good && SCH) { S.member = true; save(); ac(); if (!MUSWANT) musicStart(.4, true); ambStart(); closeVeil(enterSchool); return; }
  ...
  function arrival(first, again, quiet) {
    ...
    if (!MUSWANT) musicStart(.4, true); ambStart(); if (points()) ambFire(true);
  ...
  function heroShow(at, all) {
    ...
    backBtn(v, () => closeVeil(() => { if (homeShowing()) renderArrival(); }));
  ...
  function renderArrival() {
    ...
    TRK = null; TRK_FROM = null; CAT = null; AREA = null; if (SEARCH !== null) { SEARCH = null; paintSearchBtn(); }
    $('stage').classList.remove('room', 'searching');
  ...
  const CONDS = {
    ...
    first: (v, s) => !!s.first === v, saidYes: (v, s) => !!s.saidYes === v,
    /* what he just did — so they can talk about the SKILL rather than about
       him. "They can talk about the skill they're building right now, even if
  ...
  function pickSay(slot, ctx) {
    const all = (C.school.says || []).filter(l => l.slot === slot);
    if (!all.length) return null;
    const st = sayState(ctx); S.school.said2 = S.school.said2 || {};
    const said = S.school.said2;
    let fit = all.filter(l => {
      if (l.once && said[l.id]) return false;
      const w = l.when || {};
      return Object.keys(w).every(k => !CONDS[k] || CONDS[k](w[k], st));
    });
    if (!fit.length) return null;
    /* Weighting alone is not enough. When only one line sits at the top weight
       — "you have already done three today", say — it wins every single time
       and you hear it six arrivals running, which is exactly the scripted-robot
       feeling. So anything said in the last four turns of this slot steps
       aside, unless stepping aside would leave nothing. */
    S.school.recent = S.school.recent || {};
    const recent = S.school.recent[slot] || [];
    const fresh = fit.filter(l => recent.indexOf(l.id) < 0);
    ...
    const W = { first: 9, gapMin: 6, gapMax: 4, toNextMax: 6, runMin: 4, saidYes: 5,
      room: 5, size: 3, kind: 3,
    ...
    const pick = best[0];
    said[pick.id] = Date.now();
    S.school.recent[slot] = [pick.id].concat(recent).slice(0, 4);
    save();
    const out = { id: pick.id, who: pick.who, t: fillSay(pick.t, st) };
    /* a line can carry the other one's answer. Two of them talking is worth
       more than either of them announcing. */
    if (pick.reply) out.reply = { id: pick.reply.id, who: pick.reply.who, t: fillSay(pick.reply.t, st) };
    return out;
  }
  ...
  const asLine = l => l.who === 'marcus' ? { who: 'marcus', id: l.id, t: l.t }
    : { who: 'aurelia', id: 'ui-' + l.id, t: l.t };
  function saySlot(slot, after, ctx) {
    if (quietFolk()) { if (after) after(); return false; }
    const ln = pickSay(slot, ctx);
    if (!ln) { if (after) after(); return false; }
    speakSchool(ln.reply ? [asLine(ln), asLine(ln.reply)] : [asLine(ln)], after);
    return true;
  }
  ...
  /* He said he would come back, yesterday. That is the only thing carried
     forward — not a plan, not a chosen activity. The surprise of what is
     waiting is half of why he opens it. */
  const saidYesterday = () => { const t = S.school.tomorrow;
    return !!(t && t.said === 'yes' && t.on && dayGap(t.on, today()) === 1); };
  function askPromise() {
    const K = C.school.ask || {}; if (!K.say) return;
    const v = veil(`<div class="panel sheet askcard">
      <div class="eyebrow"><i></i>${K.title || ''}</div><h2>${K.say}</h2>
      <p class="lede">${K.lede || ''}</p>
      <div class="row"><button class="btn btn-gold" data-tm="yes" style="flex:1.3">${K.yes || 'I will be back'}</button>
        <button class="btn btn-ghost" data-tm="maybe">${K.maybe || 'We will see'}</button></div></div>`, 'light');
    backBtn(v, () => closeVeil());
    v.querySelectorAll('[data-tm]').forEach(b => b.addEventListener('click', () => { sfx('tap');
      S.school.tomorrow = { said: b.dataset.tm, on: today() }; save();
      closeVeil(() => toast(b.dataset.tm === 'yes' ? (K.gotYes || '') : (K.gotMaybe || '')));
    }));
  }
  ...
  const rerender = () => { if (homeShowing()) renderArrival(); else renderSchool(); };
  ...
  function renderAreas(list, seg) {
    $('sline').textContent = (everythingOpen() ? C.school.areasLine : C.school.areasLineFirst) || '';
    const K = C.school.kid || {};
  ...
  /* ---- stage by stage (v65) ----
     His: after the gate, only one or two ladders per life area are open, thought
     through as real programmes; the rest opens later, with a facilitator.
  ...
  function searchAll(q) {
    ...
    const rooms = everythingOpen() ? SCH.categories.filter(c => hit(c.name + ' ' + (c.line || ''))) : [];
    const ladders = [], steps = [];
    SCH.categories.forEach(c => c.tracks.filter(isOpen).forEach(tr => {
  ...
  function stepSheet(tr, st, from, where) {
    ...
    v.querySelector('#sdone').addEventListener('click', () => {
      ...
      const rc = catOf(tr) || {};
      const sayCtx = { room: rc.id, roomName: rc.name, track: tr.name,
        size: tr.size, kind: (st && st.kind) || 'skill' };
      ...
      if (won) { closeVeil(() => { rerender(); setTimeout(() => trophyShow(won), 450); }); return; }
      ...
      closeVeil(() => { rerender(); if (where !== 'track') setTimeout(() => afterStep(tr), 700); });
    });
  }
  ...
  async function boot() {
    ...
    fetch('feed.json?x=' + Date.now()).then(r => r.json()).then(f => { FEED = (f && f.posts) || []; S.feed.posts = FEED; save(); if (homeShowing()) renderArrival(); }).catch(() => {});
